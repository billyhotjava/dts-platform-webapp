...
const HISTORY = [
	{
		id: "run_2401",
		dataset: "CDP_销售订单事实表",
		performedAt: "2024-12-01 10:12",
		summary: "渠道=线上，订单数",
	},
	{
		id: "run_2402",
		dataset: "MDM_客户主数据",
		performedAt: "2024-12-02 09:30",
		summary: "客户等级=A级",
	},
	{
		id: "run_2403",
		dataset: "ODS_操作日志明细",
		performedAt: "2024-12-05 22:40",
		summary: "ERROR 级别日志",
	},
];

const SAVED_QUERIES = [
	{ id: "fav_100", name: "本月线上销售简表", dataset: "CDP_销售订单事实表", updatedAt: "2024-11-28" },
	{ id: "fav_101", name: "重点客户画像", dataset: "MDM_客户主数据", updatedAt: "2024-11-20" },
];

const PLAN_STEPS = [
	"1. Source Scan on fact.sales_orders partition(order_date >= :start_date)",
	"2. Hash Aggregate by channel, region",
	"3. Broadcast join dim.customer",
	"4. Final projection and limit",
];
...
export default function QueryWorkbenchPage() {
	const [selectedSource, setSelectedSource] = useState("CDP");
	const datasetsBySource = useMemo(
		() => DATASETS.filter((dataset) => dataset.source === selectedSource),
		[selectedSource],
	);
	...
	const availableSources = useMemo(() => Array.from(new Set(DATASETS.map((dataset) => dataset.source))), []);
	const dataSourceDatasets = DATASETS.filter((dataset) => dataset.source === selectedSource);

	const visualFields = selectedDataset?.fields ?? [];
	...
	const handleRunQuery = () => {
		...
		setIsRunning(true);
		setTimeout(() => {
			const simulatedRows = buildSampleRows(selectedDataset);
			setRunResult({
				timestamp: new Date(),
							...
							<div className="space-y-1 lg:col-span-2">
								...
								{selectedDataset ? (
									<p className="text-xs text-muted-foreground">
										库表：{selectedDataset.database}.{selectedDataset.schema}.{selectedDataset.name} · 行数估算：
				...
				<Dialog open={exportDialogOpen} onOpenChange={setExportDialogOpen}>
					<DialogContent className="max-w-md">
						...
						<div className="space-y-4 text-sm">
							...
							<div className="space-y-2">
								<Label>导出说明（审核使用）</Label>
								<Textarea placeholder="请输入导出目的、接收方等信息" />
							</div>
						</div>
						<DialogFooter>
										...
										<ul className="space-y-3 text-sm">
											{HISTORY.map((item) => (
												<li key={item.id} className="rounded-md border px-3 py-2">
													<p className="font-medium text-text-primary">{item.dataset}</p>
													<p className="text-xs text-muted-foreground">{item.performedAt}</p>
										...
										<ul className="space-y-3 text-sm">
											{SAVED_QUERIES.map((query) => (
												<li key={query.id} className="rounded-md border px-3 py-2">
													<p className="font-medium text-text-primary">{query.name}</p>
													<p className="text-xs text-muted-foreground">{query.dataset}</p>
									...
									<ScrollArea className="h-64">
										<ol className="space-y-3 text-sm">
											{PLAN_STEPS.map((step, index) => (
												<li key={step} className="rounded-md border px-3 py-2">
