...
export default function QualityRulesPage() {
	...
	const handleSubmit = () => {
		const payload: QualityRule = {
			id: form.id ?? `rule-${Date.now()}`,
			name: form.name,
			type: form.type,
			object: form.fieldIds[0] ?? form.datasetId,
			recentResult: "通过",
			owner: form.owner || "未分配",
			status: form.enabled ? "启用" : "停用",
			referenceCount: form.id ? (rules.find((rule) => rule.id === form.id)?.referenceCount ?? 0) : 0,
			frequency: form.frequency,
			config: form.config,
		};

		setRules((prev) => {
			const exists = prev.some((rule) => rule.id === payload.id);
			return exists ? prev.map((rule) => (rule.id === payload.id ? payload : rule)) : [payload, ...prev];
		});
		setDrawerOpen(false);
	};
